...
class IndividuExport implements FromCollection, WithHeadings, WithMapping, WithEvents, ShouldAutoSize
{
    use Exportable;
    public function __construct(String $stats = null, String $kates = null, String $buls = null)
    {
        $this->stats = $stats;
        $this->kates = $kates;
        $this->buls = $buls;
    ...
    */
    public function collection()
    {
        ...
        if($this->stats || $this->kates || $this->buls)
        {
            ...
            if($this->stats != "")
            {
                $result = $result->whereHas('aduan', function($query){
                    $query->where('status_aduan', $this->stats);
                });
            }
            ...
            {
                $result = $result->whereHas('aduan', function($query){
                    $query->where('kategori_aduan', $this->kates);
                });
            }
            ...
            {
                $result = $result->whereHas('aduan', function($query){
                    $query->where('bulan_laporan', $this->buls);
                });
            }

            $data = $result->where('juruteknik_bertugas', Auth::user()->id)->get();
        }
