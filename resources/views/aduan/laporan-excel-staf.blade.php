                                ...
                                @can('view technical admin')
                                    <div class="col-md-6 mt-3">
                                        ...
                                        <select class="selectfilter form-control" name="juruteknik" id="juruteknik">
                                            <option value="">Please Select</option>
                                            <option>All</option>
                                            @foreach($juruteknik as $jur)
                                                <option value="{{$jur->id}}" {{ $request->juruteknik == $jur->id  ? 'selected' : '' }}>{{$jur->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                @endcan
...
<script >
    ...
    {
        $('#tahap, #status, #kategori, #bulan, #tahun, #juruteknik').select2();

        function createDatatable(tahap = null, status = null, kategori = null, bulan = null, tahun = null, juruteknik = null)
        {
            ...
            autowidth: false,
            ajax: {
                url: "/data_juruteknikexport",
                data: {tahap:tahap, status:status, kategori:kategori, bulan:bulan, tahun:tahun, juruteknik:juruteknik},
                type: 'POST',
        ...
        $('.selectfilter').on('change',function(){
            ...
            var bulan = $('#bulan').val();
            var tahun = $('#tahun').val();
            var juruteknik = $('#juruteknik').val();
            createDatatable(tahap,status,kategori,bulan,tahun,juruteknik);
        });
