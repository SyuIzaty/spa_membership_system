...
<body>
    <table width="100%">
        <tr>
            ...
            <th style="background-color: #A8B6E3;border:1px solid black" rowspan="3">STATUS</th>
            <th style="background-color: #A8B6E3;border:1px solid black"
                colspan="{{ ($type->count() + $category->count()) * 3 }}">EQUIPMENT</th>
            <th style="background-color: #A8B6E3;border:1px solid black" rowspan="3">CONDITION</th>
            <th style="background-color: #A8B6E3;border:1px solid black" rowspan="3">REMARK</th>
        </tr>
        <tr>
        ...
        @foreach ($room as $rooms)
            <tr>
                ...
                <td style="border:1px solid black">{{ $rooms->spaceStatus->name }}</td>
                @foreach ($type as $types)
                ...
                @endforeach
                <td style="border:1px solid black">
                    @foreach ($rooms->spaceRoomConditions as $c)
                        {{ $c->spaceCondition->name ?? '-' }} - {{ $c->spaceCondition->description ?? '-' }}
                        @if ($rooms->spaceRoomConditions->count() > 0)
                            <br>
                        @endif
                    @endforeach
                </td>
